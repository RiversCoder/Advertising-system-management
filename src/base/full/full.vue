...
<script>
    ...
    export default{
        ...
        methods:{
            ...
            //初始化已经选择的文件
            initSelectFiles(){ 
                if(tool.lget('file_list_full')){ 
                    this.cresults = tool.lget('file_list_full');
                }
            },
            //初始化已经set的时间线
            initSetTimes(){
                //设置存储空间
                if(!tool.lget('time_list_full')){
                    tool.lset('time_list_full',this.setTimes);
                }else{
                    //初始化数据
                    this.setTimes = tool.lget('time_list_full');
                }
                
            },
            addFileBtn(){
                this.$router.push({'path':'/program-production/select',query:{direct:'full'}});
            },
            //添加时间和星期
            closeSure(){
        ...
        .bitem
            ...
            .bitem-day
                ...
                i 
                    font-style:normal;
    .add-bitem-btn
        @extend .block;wh(100%,65px);bgColor(#F4F4F4);bgImg('~common/images/worktime/add3.png');background-size:30px 30px;background-position:center center;cursor:pointer;border-bottom:1px solid #DEDEDE;                                             
</style>
